...
func main() {
	...
	fmt.Println(plusOne2([]int{8, 9, 9, 9}, []int{}))
}
...
func plusOne2(digits []int, add []int) []int {
	if len(digits) == 1 && digits[0] == 9 {
		return append([]int{1, 0}, add...)
	}
	n := len(digits)
	if digits[n-1]+1 < 10 {
		digits[n-1] = digits[n-1] + 1
		return append(digits, add...)
	}
	add = append(add, 0)

	return plusOne2(digits[:n-1], add)
}
